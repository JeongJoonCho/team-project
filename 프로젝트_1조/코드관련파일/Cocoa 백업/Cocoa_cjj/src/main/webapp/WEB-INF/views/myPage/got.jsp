				...
				<thead>
					<tr>
						<th colspan="4" style="text-align: center;">
							<b>${member.id }님이 받은 요청 리스트</b>
						</th>
					</tr>
					<tr>
						<td colspan="4" style="align: right;">
							<!-- 조건 조회 버튼 --> 
							<select class="me-sm-3" style="float: right; text-align: center; width: 20%;"
							id="requestStatus" onchange="selChange()">
								<option value="status" 
									<c:if test="${cri.status == 'status' }">selected</c:if>>
									전체 요청글
								</option>
								<option value="status1" 
									<c:if test="${cri.status == 'status1' }">selected</c:if>>
									대기중인 요청
								</option>
								<option value="status2" 
									<c:if test="${cri.status == 'status2' }">selected</c:if>>
									수락한 요청
								</option>
								<option value="status3"
									<c:if test="${cri.status == 'status3' }">selected</c:if>>
									거절한 요청
								</option>
								<option value="status4"
									<c:if test="${cri.status == 'status4' }">selected</c:if>>
									코칭 완료한 요청
								</option>
						</select>
					...
					<c:forEach var="gotList" items="${reqGotList}">
						<tr>
							...
							<td style="text-align: left;">
								<a style="text-decoration-line: none; color: dark; font-weight: 700;"
								href="${contextPath}/view_gotReqWait?reqNO=${gotList.reqNO}">${gotList.rTitle}</a>
							</td>
							<fmt:parseDate var="dateFmt" pattern="yyyy-MM-dd HH:mm:ss" value="${gotList.rDate}" />
